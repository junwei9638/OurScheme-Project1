...
enum Type {
    Int, String, Dot, Float, Nil, T, Quote, Symbol, LeftQuote, RightQuote
}; // enum

enum Error {
    Left, Right, Close, Eof
}; // enum
...
public:
    ...
    bool ReadSExp(){
        if( GetToken() ){
            cout << Token[ Token.size()-1 ].token << '\t' <<Token[ Token.size()-1 ].typeNum ;
            cout << endl;
            ReadSExp() ;
        } //if
